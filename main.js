...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [KAIRO-ZYNEX] ${new Date().toISOString()}: ${message}`);
}
...
async function arslanPair(number, res = null) {
    ...
    try {
        // Validasyon debaz nimewo a — anpeche kòd jenere pou yon nimewo ki two kout
        if (sanitizedNumber.length < 8) {
            if (res && !res.headersSent) {
                return res.status(400).json({ error: 'invalid_number', message: 'Numéro WhatsApp invalide (trop court).' });
            }
            return;
        }
        ...
        // Toujou itilize dènye vèsyon Baileys/WhatsApp otomatikman
        const { version } = await fetchLatestBaileysVersion();
        arslanLog(`Using Baileys/WA version: ${version.join('.')}`, 'info');
        ...
        const conn = makeWASocket({
            ...
            // ⚠️ FIX: 'Safari' konnen pou fè WhatsApp rejte pairing code yo
            // ("Impossible de connecter cet appareil"). Chrome/Ubuntu se
            // konbinezon ki dokimante kòm estab pou fonksyon pairing-code la.
            browser: Browsers.ubuntu('Chrome'),
            getMessage: async (key) => {
                const msg = await arslanStore.loadMessage(key.remoteJid, key.id);
                return msg && msg.message ? msg.message : { conversation: 'KAIRO ZYNEX' };
            }
        });
        ...
        if (!conn.authState.creds.registered) {
            ...
            try {
                // ⚠️ FIX: 3s olye 1.5s — bay websocket la tan pou l fin
                // etabli koneksyon anvan nou mande pairing code la.
                await delay(3000);
                let code = await conn.requestPairingCode(sanitizedNumber);
                // Fòmate kòd la an gwoup 4 karaktè pou li pi fasil pou li (aksyon
                // sa a pa afekte validite kòd la sou WhatsApp)
            ...
            if (res && !res.headersSent) {
                // ⚠️ FIX: pa gen `code` isit la — front-end dwe tcheke `status`
                // olye de sèlman `data.code` pou l pa afiche "SERVEUR PLEIN" a tò.
                res.json({ status: 'reconnecting', code: null, message: 'Reconnecting with existing session' });
            }
        }
   ...
   ✦ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋᴀɪʀᴏ ᴢʏɴᴇx ✦`
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363409975095814@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© KAIRO ZYNEX',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:KAIRO ZYNEX\nORG:KAIRO ZYNEX;\nTEL;type=CELL;type=VOICE;waid=50939360237:50939360237\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© KAIRO ZYNEX' } }
                    }},
        ...
        arslanLog(`KAIRO ZYNEX Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Kairo Zynex is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 KAIRO ZYNEX — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
